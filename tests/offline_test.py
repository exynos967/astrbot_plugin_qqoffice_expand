...
# 插件双开关：@module(面板)/#module(卡片) 键与指令键共存、互不污染
mem.set_plugin_panel("mod.b", False)
mem.set_plugin_card("mod.c", False)
t("panel plugin disable", mem.disabled_panel_plugins() == frozenset({"mod.b"}))
t("card plugin disable", mem.disabled_card_plugins() == frozenset({"mod.c"}))
t("plugin keys not in commands", mem.disabled_set() == frozenset({"m:c"}))
with tempfile.TemporaryDirectory() as td:
    s = OverrideStore(td)
    s.set_plugin_panel("mod.x", False)
    s.set_plugin_card("mod.y", False)
    s.set_enabled("mod.x:cmd", False)
    s2 = OverrideStore(td)
    t("dual persist reload", s2.disabled_panel_plugins() == frozenset({"mod.x"})
      and s2.disabled_card_plugins() == frozenset({"mod.y"})
      and s2.disabled_set() == frozenset({"mod.x:cmd"}))
    s2.set_plugin_panel("mod.x", True)
    t("panel re-enable keeps others", s2.disabled_panel_plugins() == frozenset()
      and s2.disabled_card_plugins() == frozenset({"mod.y"})
      and s2.disabled_set() == frozenset({"mod.x:cmd"}))
