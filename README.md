...
- 指令面板自动同步 - 收集系统与所有已激活插件的指令（含别名），幂等同步到 QQ 官方[指令面板](https://bot.q.qq.com/wiki/develop/api-v2/server-inter/menu-panel/)；指令名自动带 AstrBot 实际唤醒前缀（官方会剥离开头的 `/`，若唤醒前缀仅含 `/` 则面板指令无法触发，建议加 `#` 等符号前缀）；实测同一场景的全局面板只保留最新一个，故每场景一个面板、超 20 条按「主指令优先于别名」截断；插件装卸导致的指令变化在 15 秒内收敛；`/qqoffice_panel_sync` 可强制全量比对
