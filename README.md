...
- 指令面板自动同步 - 收集系统与所有已激活插件的指令（含别名），幂等同步到 QQ 官方[指令面板](https://bot.q.qq.com/wiki/develop/api-v2/server-inter/menu-panel/)；官方客户端面板只认「/指令」形态，故插件会把 `/` **并入内存态唤醒前缀**（不写入配置文件，等效于把面板发回的 `/xxx` 转译成 AstrBot 指令，巡检周期自动兜底热重载）；实测同一场景的全局面板只保留最新一个，故默认开启**菜单模式**（`command_panel_menu_only`）：面板只注册「菜单」入口，触发后发送可交互 markdown 卡片——插件索引 → 点插件名查看全部指令描述 → 点指令直接执行，绕开单面板 20 条上限；关闭菜单模式则直接注册指令（≤20 条，主指令优先截断）；`/qqoffice_panel_sync` 可强制全量比对
- 指令开关管理页 - 插件详情页的「指令面板注册」Page：按插件分组列出全部指令；每个插件有「面板」「卡片」两个总开关（面板=是否注册到 QQ 指令面板，卡片=是否出现在菜单卡片消息中，互不影响），每条指令还有独立开关（逐指令关 = 面板与卡片同时隐藏）；全部持久化到数据目录，变更即时触发重同步；带管理员权限过滤器的指令在面板侧标记仅管理员可点
...
| `command_panel_menu_only` | bool | true | 面板仅注册「菜单」入口，点击后发送可交互指令卡片；关闭则直接注册指令（≤20 条） |
